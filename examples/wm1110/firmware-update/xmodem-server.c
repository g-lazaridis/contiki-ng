...
uint16_t
xmodem_server_crc(uint16_t crc, uint8_t byte)
{
...
bool
xmodem_server_rx_byte(struct xmodem_server *xdm, uint8_t byte)
{
  switch(xdm->state) {
...
const char *
xmodem_server_state_name(const struct xmodem_server *xdm)
{
  return state_name(xdm->state);
}
int
xmodem_server_init(struct xmodem_server *xdm, xmodem_tx_byte tx_byte, void *cb_data)
{
  if(!tx_byte) {
    return -1;
  }
...
xmodem_server_state
xmodem_server_get_state(const struct xmodem_server *xdm)
{
  return xdm->state;
}
bool
xmodem_server_is_done(const struct xmodem_server *xdm)
{
  return xdm->state == XMODEM_STATE_SUCCESSFUL || xdm->state == XMODEM_STATE_FAILURE;
}
int
xmodem_server_process(struct xmodem_server *xdm, uint8_t *packet, uint32_t *block_num, int64_t ms_time)
{
